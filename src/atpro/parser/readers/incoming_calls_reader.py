"""Reader CSV des appels entrants.
...
class IncomingCallsReader:
    """Parse un fichier d'appels entrants vers ``Call`` / ``CallSegment``.

    :spec: FEAT-005.4
    """

    _EXPECTED_SCHEMA = "incoming_calls_v1"
    ...
    def read(self, path: Path) -> CallConsolidationResult:
        """Lit et consolide un fichier CSV.
        ...
        if schema_match.file_type not in {
            ImportFileType.INCOMING_CALLS,
            ImportFileType.UNKNOWN,
        }:
            pre_warnings.append(
                ImportWarning.create(
                    code="SCHEMA_NOT_INCOMING",
                    message=(
                        "schema detecte different de appels entrants: "
    ...
    def read_rows(
        ...
        :returns: Consolidation entrante.
        """
        raw_rows = [
        ...
        if schema_match.file_type is ImportFileType.INCOMING_CALLS:
            pass
        elif schema_match.schema_id != "unknown":
            pre_warnings.append(
                ImportWarning.create(
