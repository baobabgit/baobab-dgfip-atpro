"""Reader CSV des activites agents (format large).
...
class AgentActivitiesWideReader:
    """Parse un fichier d'activites agents en format large.

    Une ligne = un agent / un jour ; une colonne = une mesure.

    :spec: FEAT-008.1
    """

    _EXPECTED_SCHEMA = "activities_wide"
    ...
    def _build_from_rows(
        ...
        for offset, cells in enumerate(rows):
            ...
            if acc is None:
                continue
            self._builder.apply_wide_row(acc, cells, row_number=row_number)
    ...
    def _check_schema(
        ...
        *,
        ...
        """Valide le schema detecte pour le format large.
        ...
        if schema_id == "unknown":
            ...
            errors.append(
                ImportError.create(
                    code="SCHEMA_ACTIVITIES_WIDE_REQUIRED",
                    message=(
        ...
        warnings.append(
            ImportWarning.create(
                code="SCHEMA_NOT_ACTIVITIES_WIDE",
                message=(
